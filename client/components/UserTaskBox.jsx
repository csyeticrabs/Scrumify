import React, { useState } from "react";

// props.data.users.map(user => user._id === task.worker_id )

// const [userTasks, setUserTasks] = useState(props.data.users[i].totalTasks)



const UserTaskBox = (props) => {
  console.log('USERS TASK BOX IS ', props);
  const tasksArray = props.totalTasks.map(task => {return <li>{task.description}</li>})
  // props.data.users.map((user) => )
  return (
    <div class="card" style={{"width": "18rem"}}>
      <img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/70023235-8b0d-4c88-8ce2-7f3f8a9dd023/d2u5p67-3c6e1a37-957e-4061-8576-8b788d062799.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..Bro9yBOwbLht0mtr59SaHZSqRgItc-cnBqY5FSOU1OU" class="card-img-top" alt="..."/>
      <div class="card-body">
        <h5 class="card-title">{props.name}</h5>
        <ul className='card-text'>
          {tasksArray}
        </ul>
        {/* <p class="card-text">Tasks that needs to be completed?</p> */}
      </div>
    </div>
  )
};

export default UserTaskBox;
